...
Aplikasi/layanan yang mendiagnosis masalah mesin kendaraan (motor/mobil/fleet) dari audio + getaran + telemetri menggunakan model deep learning. Hasil: jenis kerusakan, tingkat keparahan, rekomendasi tindakan, dan estimasi biaya servis
...
pastikan anda menginstall node.js lalu cek dengan cmd/bash :
...
pastikan ffmpeg di install

install semua yang ada di requirements.txt bash :

pip install -r requirements.txt

Panduan Menjalankan Aplikasi Berikut adalah langkah-langkah yang diperlukan untuk menjalankan aplikasi (baik backend maupun frontend):
...
Menjalankan Layanan Frontend Setelah backend berjalan, Anda dapat menjalankan frontend.
